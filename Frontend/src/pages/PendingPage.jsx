        ...
        <div className={layoutClasses.headerWrapper}>
            <div>
                <h1 className='text-2xl md:text-4xl font-bold text-gray-800 flex items-center gap-2'>
                    <ListChecks className='text-purple-500 ml-3' /> Pending Task
                </h1>
        ...
        <div className='space-y-4'> 
            {sortedPendingTasks.length === 0 ? (
              <div className={layoutClasses.emptyState}>
                   <div className='max-w-xs mx-auto py-6'>
                        ...
                        <p className=' text-sm text-gray-500 mt-4'>
                            No pending task found - great work
                        </p>
              ...
              sortedPendingTasks.map((task) => (
                <TaskItem key={task._id || task.id} task={task} showCompleteCheckbox onDelete={() => handleDelete(task._id || task.id)} onToggleComplete={() => handleToggleComplete(task._id || task.id)} onEdit={() => { setSelectedTask(task); setShowModal(true)}} onRefresh={fetchTasks} />
              ))
            )}
        </div>

        <TaskModal isOpen={!!selectedTask || showModal} onClose={() => { setShowModal(false); setSelectedTask(null); }} taskToEdit={selectedTask} projects={projects} onTaskCreated={fetchTasks} />
    </div>
