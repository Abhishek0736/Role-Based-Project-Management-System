...
export const menuItems = [
    ...
    { text: "Projects", path: "/projects", icon: <FolderOpen className="w-5 h-5" /> },
    { text: "Team", path: "/team", icon: <Users className="w-5 h-5" /> },
    ...
    { text: "Analytics", path: "/analytics", icon: <BarChart3 className="w-5 h-5" /> },
    { text: "Calendar", path: "/calendar", icon: <Calendar className="w-5 h-5" /> },
    { text: "Time Tracking", path: "/time-tracking", icon: <Clock className="w-5 h-5" /> },
]
...
export const SIDEBAR_CLASSES = {
    ...
    mobileDrawer: "absolute top-0 left-0 w-64 h-600 bg-white/90 backdrop-blur-md border-r border-purple-100 shadow-lg z-50 p-4 flex flex-col space-y-6",
}
...
export const STATS_GRID = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb- md:mb-6"
...
export const CT_CLASSES = {
    ...
    subtitle: "text-xs md:text-sm text-gray-500 mt-1 ml-12 md:ml-8",
...
// APP.JSX
// `data` may not be defined in this module. Guard it to avoid ReferenceError.
const user = {
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent((typeof data !== 'undefined' && data && data.name) ? data.name : 'User')}&background=random`
};
